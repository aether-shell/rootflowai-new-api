...
func (u *Usage) NormalizeCacheWriteTokens() {
	...
	for _, candidate := range candidates {
		...
		if candidate.reported && candidate.tokens > maxReportedTokens {
			maxReportedTokens = candidate.tokens
		}
		if candidate.tokens > maxTokens {
			maxTokens = candidate.tokens
		}
	}
	if maxReportedTokens > u.PromptTokensDetails.CacheWriteTokens {
		u.PromptTokensDetails.CacheWriteTokens = maxReportedTokens
	}
	if u.PromptTokensDetails.CachedCreationTokens <= 0 && maxTokens > 0 {
