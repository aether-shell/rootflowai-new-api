...
export const getModelBillingModeLabel = (mode?: string) => {
  if (mode === 'task') return 'Task'
  if (mode === 'second') return 'Second'
  return 'Default'
}

export const getModelBillingModeVariant = (
  mode?: string
): 'warning' | 'info' | 'success' => {
...
export const buildModelSnapshots = ({
  ...
  return Array.from(modelNames).map((name) => {
    ...
    const rawModelBillingMode = modelBillingModeMap[name]
    const normalizedModelBillingMode =
      rawModelBillingMode === 'task' || rawModelBillingMode === 'second'
        ? rawModelBillingMode
        : undefined
...
export const getSnapshotSignature = (snapshot?: ModelPricingSnapshot) => {
  ...
  return JSON.stringify({
    ...
    modelBillingMode: snapshot.modelBillingMode || '',
