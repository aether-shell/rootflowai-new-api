...
func TestNewOpenAIBillingUsageRecognizesCacheWriteAliases(t *testing.T) {
	tests := []struct {
		name  string
		usage *Usage
	}{
		{name: "top-level cache creation", usage: &Usage{CacheCreationTokens: 1}},
		{name: "top-level cache write", usage: &Usage{CacheWriteTokens: 1}},
		{name: "top-level cache creation input", usage: &Usage{CacheCreationInputTokens: 1}},
		{name: "top-level cache write input", usage: &Usage{CacheWriteInputTokens: 1}},
		{name: "prompt details cache creation", usage: &Usage{PromptTokensDetails: InputTokenDetails{CacheCreationTokens: 1}}},
		{name: "prompt details cache write", usage: &Usage{PromptTokensDetails: InputTokenDetails{CacheWriteTokens: 1}}},
		{name: "responses details cache creation", usage: &Usage{InputTokensDetails: &InputTokenDetails{CacheCreationTokens: 1}}},
		{name: "responses details cache write", usage: &Usage{InputTokensDetails: &InputTokenDetails{CacheWriteTokens: 1}}},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			billingUsage := NewOpenAIResponsesBillingUsage(test.usage)
