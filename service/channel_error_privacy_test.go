...
func TestChannelErrorForUserClassifiesMultilingualContentAudit(t *testing.T) {
	tests := []struct {
		...
		{name: "English content policy", statusCode: 403, message: "Request blocked by content policy", wantPublic: true},
		{name: "English structured safety", statusCode: 400, message: "request rejected", typeCode: "prompt_blocked", wantPublic: true},
		{name: "localized unknown forbidden", statusCode: 403, message: "forbidden by provider", wantPublic: true},
