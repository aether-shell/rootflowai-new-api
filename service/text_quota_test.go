...
func TestEffectiveBillingUsageNormalizesOpenAICacheWriteAliases(t *testing.T) {
	usage := &dto.Usage{
	...
	require.Equal(t, 1000, effective.PromptTokens)
	require.Equal(t, 50, effective.CompletionTokens)
	require.Equal(t, 200, effective.PromptTokensDetails.CachedCreationTokens)
	require.True(t, effective.CacheWriteTokensReported)
}
...
func TestCalculateTextQuotaSummaryUsesConfiguredOpenAICacheWriteRatio(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	...
	tests := []struct {
		...
		{name: "explicit GPT-5.6 ratio", configured: true, wantQuota: 3150, wantRatio: 1.25},
		{name: "unconfigured older OpenAI model", configured: false, wantQuota: 3025, wantRatio: 1},
	}
		...
		t.Run(test.name, func(t *testing.T) {
			relayInfo := &relaycommon.RelayInfo{
				...
				PriceData: types.PriceData{
					...
					CacheRatio:                   0.1,
					CacheCreationRatio:           1.25,
					CacheCreationRatioConfigured: test.configured,
					GroupRatioInfo:               types.GroupRatioInfo{GroupRatio: 1},
				},
				StartTime: time.Now(),
			}
			...
			require.Equal(t, test.wantRatio, summary.CacheCreationRatio)
			require.Equal(t, test.wantQuota, summary.Quota)
		})
	}
}

func TestCalculateTextQuotaSummaryClampsOverlappingOpenAICacheWriteBreakdown(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	relayInfo := &relaycommon.RelayInfo{
	...
	require.Equal(t, 46, summary.Quota)
}
